...
void ENET_SetIRQ(ENET_Type *base,uint32_t interrupt,bool enable){
	uint32_t instance = ENET_GetInstance(base);

	if(enable){
		if (interrupt & ENET_TX_INTERRUPT)
	    {
	        EnableIRQ(s_enetTxIrqId[instance]);
	    }
	    if (interrupt & ENET_RX_INTERRUPT)
	    {
	        EnableIRQ(s_enetRxIrqId[instance]);
	    }
	    if (interrupt & ENET_ERR_INTERRUPT)
	    {
		...
		if (interrupt & ENET_TX_INTERRUPT)
	    {
	        DisableIRQ(s_enetTxIrqId[instance]);
	    }
	    if (interrupt & ENET_RX_INTERRUPT)
	    {
	        DisableIRQ(s_enetRxIrqId[instance]);
	    }
	    if (interrupt & ENET_ERR_INTERRUPT)
	    {
	        DisableIRQ(s_enetErrIrqId[instance]);
	    }
	}
}
