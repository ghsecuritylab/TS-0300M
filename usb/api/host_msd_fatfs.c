 ...
 */
void USB_HostMsdCallback(void *param, uint8_t *data, uint32_t dataLength, usb_status_t status);
 ...
 * @param msdFatfsInstance   the host fatfs instance pointer.
 */
void USB_HostMsdFatfsTest(usb_host_msd_fatfs_instance_t *msdFatfsInstance);
...
//	usb_echo("void USB_HostMsdControlCallback");
...
//    }
    ...
    usb_echo("create file \"f_1.txt\"......");
    fatfsCode = f_open(&file, _T("1:/f_1.txt"), FA_WRITE | FA_READ | FA_CREATE_ALWAYS);
    if (fatfsCode)
    {
        if (fatfsCode == FR_EXIST)
        {
...
void USB_HostMsdTask(void *arg)
{
    ...
    if (msdFatfsInstance->deviceState != msdFatfsInstance->prevDeviceState)
    {
        ...
        case kStatus_DEV_Attached: /* deivce is attached and numeration is done */
            usb_echo("USB_HostMsdTask:kStatus_DEV_Attached\r\n");
        ...
        case kStatus_DEV_Detached: /* device is detached */
            usb_echo("USB_HostMsdTask:kStatus_DEV_Detached\r\n");
    ...
    case kUSB_HostMsdRunSetInterface: /* set msd interface */
        ...
        status = USB_HostMsdSetInterface(msdFatfsInstance->classHandle, msdFatfsInstance->interfaceHandle, 0,
                                         USB_HostMsdCallback, msdFatfsInstance);
        if (status != kStatus_USB_Success)
        {
...
////					usb_echo("\r\n ***************** \r\nUSB_HostMsdTask:USB_HostMsdFatfsTest\r\n ******************** \r\n");
        ...
        if (g_MsdFatfsInstance.configHandle == configurationHandle)
        {
            if ((g_MsdFatfsInstance.deviceHandle != NULL) && (g_MsdFatfsInstance.interfaceHandle != NULL))
            {
                ...
                if (g_MsdFatfsInstance.deviceState == kStatus_DEV_Idle)
                {
                    ...
                    */
                    status = USB_HostMsdInit(g_MsdFatfsInstance.deviceHandle,&g_MsdFatfsInstance.classHandle); /* msd class initialization */
                    ...
                    g_MsdFatfsInstance.runState = kUSB_HostMsdRunSetInterface;
                    /**************/
                }
                else
                {
        ...
        if (g_MsdFatfsInstance.configHandle == configurationHandle)
        {
            ...
            if (g_MsdFatfsInstance.deviceState != kStatus_DEV_Idle)
            {
                ...
                */
                ...
                g_MsdFatfsInstance.runState = kUSB_HostMsdRunFSUnmount;
                USB_HostMsdDeinit(g_MsdFatfsInstance.deviceHandle,g_MsdFatfsInstance.classHandle); /* msd class de-initialization */
