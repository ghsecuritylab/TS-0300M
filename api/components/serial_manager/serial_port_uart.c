...
#if (SERIAL_PORT_USE_WITHC_HAL_UART == SDK_HAL_UART)	
#include "uart.h"
#elif (SERIAL_PORT_USE_WITHC_HAL_UART == CUSTOM_HAL_UART)	
#include "hal_uart.h"
#endif
...
serial_manager_status_t Serial_UartInit(serial_handle_t serialHandle, void *serialConfig)
{
	...
	LPUART_Type * lpuartBase[] = LPUART_BASE_PTRS;
#endif
	...
	config.base = lpuartBase[uartConfig->instance];
	config.baudRate = uartConfig->baudRate;
	config.enableRx = uartConfig->enableRx;
    config.enableTx = uartConfig->enableTx;
	config.rxIrq = false;
	config.txIrq = false;
	
	if (kStatus_Success != HAL_UartInit(((HAL_UartHandler_S)&serialUartHandle->usartHandleBuffer[0]), &config))
    {
        return kStatus_SerialManager_Error;
    }
#endif
...
#elif (SERIAL_PORT_USE_WITHC_HAL_UART == CUSTOM_HAL_UART)	
	HAL_UartDeinit(((HAL_UartHandler_S)&serialUartHandle->usartHandleBuffer[0]));
#endif
...
serial_manager_status_t Serial_UartWrite(serial_handle_t serialHandle, uint8_t *buffer, uint32_t length)
{
    ...
    return (serial_manager_status_t)HAL_UartSendBlocking(((hal_uart_handle_t)&serialUartHandle->usartHandleBuffer[0]),
                                                         buffer, length);
#elif (SERIAL_PORT_USE_WITHC_HAL_UART == CUSTOM_HAL_UART)	
	return (serial_manager_status_t)HAL_UartSend(((HAL_UartHandler_S)&serialUartHandle->usartHandleBuffer[0]),
                                                         buffer, length);
#endif
...
#elif (SERIAL_PORT_USE_WITHC_HAL_UART == CUSTOM_HAL_UART)	
	return (serial_manager_status_t)HAL_UartReceive(((HAL_UartHandler_S)&serialUartHandle->usartHandleBuffer[0]), 
													buffer, length);
#endif
