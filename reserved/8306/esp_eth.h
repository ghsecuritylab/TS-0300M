 ...
 *      - ESP_FAIL
 */
 #define EMAC_RX_BUF_USE_TASK		1
 
esp_err_t esp_eth_tx(uint8_t *buf, uint16_t size);
uint8_t * emac_get_current_txchan(void);//20180326
void emac_setup_current_txchan(uint16_t size);//20180326
void emac_tx_enter_spinlock(void);
void emac_tx_exit_spinlock(void);
...
/**
 * @brief  set dst macaddr filter.
 */
void emac_set_dst_macaddr_filter(uint8_t filter_num, uint8_t *mac);
void emac_clr_dst_macaddr_filter(uint8_t filter_num);
