...
static int32_t httpsrv_set_params(HTTPSRV_STRUCT *server, HTTPSRV_PARAM_STRUCT *params)
{
    ...
    if (params != NULL)
    {
        ...
        if (server->params.address.sa_family == AF_INET)
        {
			...
			/* 增加可定义IP(因为原来默认IP为0，lwip绑定ip为0的时候单网口状态下
			实际没有问题，但双网口不指定ip会有访问两个网口的ip都能打开网页的情况) */
			if(((struct sockaddr_in *)(&params->address))->sin_addr.s_addr)
				((struct sockaddr_in *)(&server->params.address))->sin_addr =
                    ((struct sockaddr_in *)(&params->address))->sin_addr;
        }
#endif
#if LWIP_IPV6
        else if (server->params.address.sa_family == AF_INET6)
        {
			...
			/* 增加可定义IP（同IPV4）*/ 
			if (((struct sockaddr_in6 *)(&params->address))->sin6_addr.s_addr)
                ((struct sockaddr_in6 *)(&server->params.address))->sin6_addr =
                    ((struct sockaddr_in *)(&params->address))->sin6_addr;
        }
#endif
        else
        {
